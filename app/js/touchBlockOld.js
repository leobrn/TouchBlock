(function (win, doc) {
    ...
    const trfRegExp = /[-0-9.]+(?=px)/,
        ...
        getElement = function (event, nameElement) {
            const element = event.target.closest(`#${nameElement}`)
            return element
        },
    ...
    const TouchBlock = function (options) {
        const settings = {
            ...
            touchStart = function (event) {
                const elEvent = getElement(event, this.settings.elementID)
                if (!elEvent) { return }
        ...
        if (settings.isSlider) {
            parentBlock = doc.getElementById(settings.elementID)
        ...
        cache.paginationElement = parentBlock ? parentBlock.querySelector(`.touch-block__pagination`) : undefined
